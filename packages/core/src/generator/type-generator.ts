...
function renderObjectType(
  ...
  options: { asInterface?: boolean; fallback?: string } = {},
): string {
  ...
  if (propertyEntries.length === 0) {
    ...
    return options.fallback ?? 'Record<string, unknown>';
  }
...
export function renderSchema(schema: SchemaLike, context: TypeRenderContext): string {
  ...
  if (!schema.type) {
    ...
    if (resolved !== schema) {
      return renderSchema(resolved, context);
    }
  }
...
function renderNamedSchemaExport(
  ...
  if (resolved.type === 'object' || resolved.properties || resolved.additionalProperties) {
    return `export interface ${toTypeName(name)} ${renderObjectType(resolved, context, { asInterface: true })}`;
  }

  return `export type ${toTypeName(name)} = ${renderSchema(resolved, context)};`;
}

function createRequestSchema(operation: ParsedOperation): OpenAPIV3.SchemaObject | undefined {
  const properties: Record<string, SchemaLike> = {};
