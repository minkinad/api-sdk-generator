...
1. Установи **Node.js 24** и pnpm 9.15.4. Если используешь nvm:
   ...
   Для запуска самого генератора достаточно Node.js 20+. Node.js 24 рекомендуется
...
4. Опубликуй:
   ...
   `packages/*/package.json`; уже существующие версии пропускаются. При ошибке
...
5. Проверь результат:
   ...
   Оба пакета должны появиться в профиле `minkinad`.
